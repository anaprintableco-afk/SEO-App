...
st.markdown("""
    <style>
        ...
        .stButton>button { background: linear-gradient(90deg, #FF5A1F, #FF8C00); color: white; border: none; font-weight: bold; width: 100%; }
    </style>
...
# ب) پنل کاربری (لاگین موفقیت‌آمیز)
    ...
    with st.sidebar:
        st.markdown('<div class="logo-text"><span class="logo-icon">A</span> AtlasRank</div>', unsafe_allow_html=True)
        if user_info:
            st.write(f"Welcome, **{user_info.get('given_name', 'User')}** 👋")
